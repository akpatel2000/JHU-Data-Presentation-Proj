...
shinyServer(function(input, output) {
  ...
  g2 <- reactive({
    ...
    output$Documentation <- renderText("How to run Treasury Curve Application?
...
> The yields provided by the Federal Reserve are for only the 2, 5, 7, 10 and 30 year maturities.  I use a function called spline() to produce a yield curve.
...
> The original data is found on the US Federal Reserve Database.  The comma delimited files were downloaded and are hosted on my github account.  As a result there may be some delay in displaying the plots, depending on internet speed.
")
